...
//                detail.setSongList(singerRepository.findBySingerId(detail.getSingerId()).get().getSongs());
			...
			TimbreAnalysis newTimbreAnalysis = TimbreAnalysis.builder().userId(userId)
					...
					.rmsVar(((Double) result.get("rms_mean")).floatValue()).singer1(topFive.get(0).getSingerId())
					.singer2(topFive.get(1).getSingerId()).singer3(topFive.get(2).getSingerId())
					.singer4(topFive.get(3).getSingerId()).singer5(topFive.get(4).getSingerId())
					...
					.similarity5(topFive.get(4).getSimilarityPercent()).time(LocalDateTime.now()).build();
			...
			TimbreAnalysisResp timbreAnalysisResp = TimbreAnalysisResp.builder()
					.mfccMean(saveTimbreAnalysis.getMfccMean()).stftMean(saveTimbreAnalysis.getStftMean())
					.zcrMean(saveTimbreAnalysis.getZcrMean()).spcMean(saveTimbreAnalysis.getSpcMean())
					.sprMean(saveTimbreAnalysis.getSprMean()).rmsMean(saveTimbreAnalysis.getRmsMean())
					.mfccVar(saveTimbreAnalysis.getMfccVar()).stftVar(saveTimbreAnalysis.getStftVar())
					.zcrVar(saveTimbreAnalysis.getZcrVar()).spcVar(saveTimbreAnalysis.getSpcVar())
					.sprVar(saveTimbreAnalysis.getSprVar()).rmsVar(saveTimbreAnalysis.getRmsVar())
					.timbreId(saveTimbreAnalysis.getTimbreId()).time(saveTimbreAnalysis.getTime())
	...
	public TimbreAnalysisResp selectOneTimbreAnalysis(Long timbreId) {
		...
		List<SingerDetailResp> topFive = new ArrayList<>();
		topFive.add(new SingerDetailResp(singerRepository.findBySingerId(timbreAnalysis.getSinger1()).get(),
				timbreAnalysis.getSimilarity1()));
		topFive.add(new SingerDetailResp(singerRepository.findBySingerId(timbreAnalysis.getSinger2()).get(),
				timbreAnalysis.getSimilarity2()));
		topFive.add(new SingerDetailResp(singerRepository.findBySingerId(timbreAnalysis.getSinger3()).get(),
				timbreAnalysis.getSimilarity3()));
		topFive.add(new SingerDetailResp(singerRepository.findBySingerId(timbreAnalysis.getSinger4()).get(),
				timbreAnalysis.getSimilarity4()));
		topFive.add(new SingerDetailResp(singerRepository.findBySingerId(timbreAnalysis.getSinger5()).get(),
				timbreAnalysis.getSimilarity5()));
